public class mergeSort {
	...
	public static void merge(int[] a, int start, int mid, int end)
    {
        ...
        for (int i = 0; i < l1; ++i) {
            fir[i] = a[start + i]; 
            System.out.println("Adding " + fir[i] + " to the first array.");
        }
        for (int j = 0; j < l2; ++j) {
            sec[j] = a[mid + 1 + j]; 
            System.out.println("Adding " + sec[j] + " to the second array.");
        }
        int i = 0;
        int j = 0;
        ...
        for (int s = start; s < a.length; s++) {
        	...
        	if (i < l1 && j < l2) {
        		...
        		if (fir[i] <= sec[j]) {
                    ...
                    a[s] = fir[i];
                    System.out.println("Adding " + a[s] + " to the main array a.");
                    i++;
    ...
    public static void sort(int[] a, int start, int end)
    {
    	if (start == end) {
            ...
            System.out.println("The middle index is " + mid);
            // call the recursive merge sort method and split it in half
            sort(a, start, mid);
            System.out.println("The start index of the first subarray is " + start + " and the end index is " + mid);
            sort(a, mid + 1, end);
            System.out.println("The start index of the second subarray is " + (mid + 1) + " and the end index is " + end);
